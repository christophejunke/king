...
Finally, when no argument is given, the closure returns a fresh vector
of all the given already seen by the closure, stored in the same order
as the index which was associated with the value. Note that those
values are the one being that were passed to the function, not the
     ...
     (lambda (&optional (data nil datap))
       (if datap
           ...
           (let ((array (make-array (hash-table-count map) :adjustable nil)))
             ...
             array)))
     ;; secondary value, compute vector of keys instead
     (lambda ()
       (let ((array (make-array (hash-table-count map) :adjustable nil)))
         (maphash (lambda (k v)
                    (setf (aref array (cdr v)) k))
                  map)
         array)))))
